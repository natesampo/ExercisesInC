...
{
    // Recursively search the list by going to the next element and checking for equality with key

    if(list == NULL) {
        return NULL;
    }

    if(key->equal(list->key, key)) {
        return list->value;
    }
...
{
    int index = key->hash(key)%map->n;
    Node* node = make_node(key, value, map->lists[index]);
    map->lists[index] = node;
}
...
{
    return list_lookup(map->lists[key->hash(key)%map->n], key)
}
